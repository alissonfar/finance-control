...
window.APP.controllers.cartoes = {
       ...
       APP.db.all(sql, [], (err, cartoes) => {
           ...
           cartoes.forEach(cartao => {
               ...
               tr.innerHTML = `
                   ...
                   <td>
                       <button onclick="APP.controllers.cartoes.gerarFatura(${cartao.id})">Gerar Fatura</button>
                       <button onclick="APP.controllers.cartoes.verFaturas(${cartao.id})">Ver Faturas</button>
                   </td>
               `;
   ...
   verFaturas: function(cartaoId) {
       ...
       APP.db.all(sql, [cartaoId], (err, faturas) => {
           ...
           let html = `
               <div id="modalFaturas" class="modal">
           ...
           faturas.forEach(fatura => {
               html += `
                   <tr>
                       ...
                       <td>${APP.utils.formatarMoeda(fatura.valor_total)}</td>
                       <td>${fatura.status}</td>
                   </tr>
               `;
           });
           ...
           html += `
                       ...
                       <button onclick="document.getElementById('modalFaturas').remove()">Fechar</button>
                   </div>
               </div>
           `;
